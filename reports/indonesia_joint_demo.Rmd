...
<small> *The code below constructs three simulation variants.
1.No-shock baseline, where GDP grows steadily without disasters;
...
Each scenario is run 500 times using the a Monte Carlo engine, which links sovereign-level debt and GDP shocks to firm-level cash flows (FCF). Then every scenario is benchmarked against the no-shock baseline by year, computing loss-based KPIs: the 95th-percentile shortfall (VaR₉₅), tail conditional shortfall (TVaR₉₅), probability of negative cash flow (P(FCF < 0)), and the expected annual shortfall (EAS).* <small>
...
joint_base <- run_joint_multiple(
  ...
  seed0 = 42, 
  base_rev = 1.1e9, base_cogs = 6.8e8, base_opex = 2.1e8, base_rate = .06,
...
baseline <- run_joint_multiple(
  n = 200, params_country = params_noshock, company_assets = assets,
  base_rev = 1.1e9, base_cogs = 6.8e8, base_opex = 2.1e8, base_rate = .06,
  demand_elasticity = 1.2,
  intensity_sampler = function() 0
) %>% group_by(year) %>% dplyr::summarise(FCF_base = median(FCF, na.rm=TRUE), .groups="drop")

baseline_as_joint <- baseline |> dplyr::transmute(year, FCF = FCF_base)
...
calc_scenario_metrics <- function(joint_df, baseline_df) {
  stopifnot("year" %in% names(joint_df))
  stopifnot("FCF"  %in% names(joint_df))
  ...
  # Ensure the join key has the same type on both sides
  joint_df <- dplyr::mutate(joint_df, year = as.integer(year))
  base_by_year  <- dplyr::mutate(base_by_year,  year = as.integer(year))

  df <- joint_df |>
    dplyr::left_join(base_by_year, by = "year")
  
  if (!("FCF_base" %in% names(df))) {
    stop("Join failed: 'FCF_base' is missing after left_join(). ",
         "Check that 'baseline_df' really has (year, FCF_base) and that 'year' types match.")
  }
  
  df <- df %>% dplyr::mutate(shortfall = FCF - FCF_base)   # negative is bad
...
# ---- metrics for Baseline vs Adaptation vs no shock ----
m_base  <- calc_scenario_metrics(joint_base,  baseline)
m_adapt <- calc_scenario_metrics(joint_adapt, baseline)
m_noshock <- calc_scenario_metrics(baseline_as_joint, baseline)
...
kpis_tbl_num <- dplyr::bind_rows(
   tibble::tibble(
    Scenario =         "No-shock baseline",
    P_lt0 =             m_noshock$p_breach,
    VaR95 =             m_noshock$VaR95,
    TVaR95 =            m_noshock$TVaR95,
    EAS =               m_noshock$EAS,
    NPV_benefit =       NA_real_,
    NPV_cost =          NA_real_,
    ROI =               NA_real_,
    Payback_yr =        NA_real_
  ),
...
sf_noshock <- baseline_as_joint$FCF - baseline$FCF_base
ep_n <- make_ep(sf_noshock); ep_n$Scenario <- "No-shock"
ep_all <- dplyr::bind_rows(ep_b, ep_a, ep_n)
...
* The EP (Exceedance Probability) curve shows the distribution of annual FCF shortfalls across 500 Monte Carlo runs.
The curve’s shape highlights how financing spreads amplify downside risk: under baseline conditions, higher sovereign leverage pushes firm borrowing costs upward, deepening rare-event losses (steeper left tail).
Adaptation compresses this tail — fewer severe shortfalls remain, and the 95th percentile (VaR) loss shrinks by ~25%, demonstrating stabilizing physical cashflow and 
limited credit spread transmission from macro stress to corporate balance sheet.*
